...
const server = net.createServer((socket) => {
  ...
  socket.write(
    "Bem-vindo! Você gostaria de fazer login (L) ou se cadastrar (C)?"
  );
  ...
  socket.on("data", (data) => {
    ...
    if (!isRegistering) {
      ...
      if (message.toUpperCase() === "L") {
        socket.write(
          "Por favor, digite sua matrícula e senha separados por vírgula:"
        );
      } else if (message.toUpperCase() === "C") {
        isRegistering = true;
        socket.write(
          "Por favor, digite uma nova matrícula e senha separados por vírgula:"
        );
      } else {
        socket.write(
          "Opção inválida. Você gostaria de fazer login (L) ou se cadastrar (C)?"
        );
      }
    } else {
    ...
    // Caso contrário, peça que o usuário se cadastre.
      ...
      socket.write(
        "Você não está registrado. Por favor, digite sua matrícula e senha separados por vírgula:"
      );
    }
  });
